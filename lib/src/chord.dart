...
class ChordPattern {
  ...
  static final Map<String, ChordPattern> _byIntervals = <String, ChordPattern>{};

  ChordPattern({String this.name, String this.fullName, List<String> this.abbrs, List<Interval> this.intervals}) {
    _byName[name] = this;
    _byName[fullName] = this;
    for (var abbr in abbrs) { _byName[abbr] = this; }
  ...
  static void _initializeChords() {
    ...
    for (var spec in ChordPatternSpecs) {
      ...
      var intervals = spec['intervals'].split('').map((c) {
        ...
        if (semitones == null) { semitones = int.parse(c); }
...
final List ChordPatternSpecs = [
  ...
  {"name": 'Dominant 7♭5', "abbrs": ['7♭5'], "intervals": '046t'},
  // following is also half-diminished 7th
  {"name": 'Minor 7th ♭5', "abbrs": ['ø', 'Ø', 'm7♭5'], "intervals": '036t'},
